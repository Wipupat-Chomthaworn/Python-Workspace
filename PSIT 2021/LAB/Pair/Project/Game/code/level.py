...
from tiles import Tile, StaticTile , Crate, AnimatedTile , Coin, Tree
...
class Level:
    def __init__(self, level_data, surface):
        ...
        #foreground trees
        tree_layout = import_csv(level_data['tree'])
        self.tree_sprites = self.create_tile_group(tree_layout, 'tree')

        #background trees
        tree_layout = import_csv(level_data['bg trees'])
        self.bg_tree_sprites = self.create_tile_group(tree_layout, 'bg trees')
        ...
        #contraint
        contraint_layout = import_csv(level_data['constraints'])
        self.constraint_sprites = self.create_tile_group(contraint_layout,'constraint')
        ...
        for row_index, row in enumerate(layout):
            for col_index,val in enumerate(row):
                if val != '-1':
                    ...
                    if type == 'tree':
                        if val == '2':
                            sprite = Tree(tile_size, x, y, 'graphics/terrain/tree_small', 50)
                        if val == '3':
                            sprite = Tree(tile_size, x, y, 'graphics/terrain/tree_large', 50)
    ...
    def horizontal_movement_collision(self):
        ...
        object_sprites = self.terrain_sprites.sprites() + self.crate_sprites.sprites()
